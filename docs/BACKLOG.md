...
- [x] CI: 125 unit tests + REAL e2e on Docker — green for consecutive runs; self-hosted runner as a Windows service (NSSM); runner paused during local demos (runs queue, no stack teardown)
...
- [x] Repository published: riznykst/fen-triadic-synthesis (13 commits at publish — 2026-08-28 —, 127 by 2026-09-02, 173 by 2026-09-06; public since 2026-09-03)
...
- [x] CI e2e honesty (2026-09-12, `2d6f009` → `676bf6f`): the smoke test's consumer-group readiness guard actually runs now — kafka-python 2.3.x returns a LIST of `GroupInformation` (the probe assumed a dict), "a member exists" is not "ready" (`Stable` + a real assignment, with a settle before publishing), and the decoded `member_assignment` has no `len()`; the `e2e` job also FAILS instead of skipping when the Docker daemon is unavailable, because a skipped e2e used to be reported as a green run. Verified in run `34690487809` (all steps executed, guard logged `(Stable + assigned): ready` three times)
- [x] Companion paper published (2026-09-12): Riznyk, V., *Decentralised Agentic Governance…*, CERN IdeaSquare Journal of Experimental Innovation, [DOI 10.23726/cij.2026.1829](https://doi.org/10.23726/cij.2026.1829) — README citation/DOI badge and `CITATION.cff` (`preferred-citation`, affiliation, version 0.2.2) updated from "accepted, in press"
...
- [x] UI e2e test — DONE (2026-09-03): Playwright suite `web/e2e/ui.spec.js` (portal load/no-JS-errors, submit → SSE validated flip, triadic scaffold, widget demo) against the live stack; wired into the CI `e2e` job; `@playwright/test` pinned as web devDependency
