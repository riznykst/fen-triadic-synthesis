...
from services.fen_bridge.kafka_io import make_consumer, poll_batch
...
def run(config: FenBridgeConfig, client: FenClient, consumer) -> None:
    ...
    """
    batch = list(poll_batch(consumer, config.batch_size, config.poll_timeout_ms))
    if not batch:
        return
    client.submit_candidates(batch)
